function H = f_rdaPlot(result,Y,wascores,scale,offset,yLabels,xLabels,fmt,iter,sLabels,bnd)
% - ordination distance biplot for a Redundancy Analysis (RDA)
%
% USAGE: H = f_rdaPlot(result,Y,wascores,scale,offset,yLabels,xLabels,'fmt',iter,sLabels,bnd)
%
...
%            e.g., sLabels = {'A' 'B' 'C'};
% bnd      = scale scores to fit within -1 to 1 bounds             (default = 0)
%
%
...
%  .p          = p-value of correlation of expl. variables with canonical axes
%  .siteScores = site scores
%  .hdl        = handle to plotted siteScores
%
...
% Feb-2015: site labels now correctly formatted
% Jul-2015: now supports 'bnd' option to scale site & fitted scores; now
%           collects handle to plotted site scores
% Feb-2016: replaced null with ~
...
if (nargin < 11), bnd     = 0;     end % default no site labels
...
% Scale to fit within -1 to 1 bounds (after eq. 1.10 in L&L, 1998):
if (bnd>0)
   
   siteScores = (siteScores./repmat(max([max(abs(siteScores(:,1)));max(abs(siteScores(:,2)))]),...
      size(siteScores)));
   
   fitScores = (fitScores./repmat(max([max(abs(fitScores(:,1)));max(abs(fitScores(:,2)))]),...
      size(fitScores)));
end
% ----------------------------------
...
if iter>0
   ...
   for j=1:ncX
      for k = 1:2
         ...
         [~,p(j,k)] = f_corr(X(:,j),fitScores(:,k),0,iter); % correlation with site scores
      end
   end
else
   p = NaN;
...
% Plot siteScores:
if isempty(sLabels)
   hdl = plot(siteScores(:,1),siteScores(:,2),'o','MarkerFaceColor',0.70*[1 1 1],...
      'MarkerEdgeColor','none');
else
   hdl = plot(siteScores(:,1),siteScores(:,2),'.w','MarkerFaceColor',0.70*[1 1 1],...
      'MarkerEdgeColor','none');
...
H.hdl        = hdl; % handle to plotted siteScores
