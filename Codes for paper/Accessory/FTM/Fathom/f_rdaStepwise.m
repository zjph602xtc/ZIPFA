function [glob,cond,model] = f_rdaStepwise(Y,X,iter,verb,alpha,stop,grp,xLabels)
% - stepwise selection of explanatory variables in RDA based on F-stat
%
% USAGE: [glob,cond,model] = f_rdaStepwise(Y,X,iter,verb,alpha,stop,grp,xLabels)
%
...
% stop  = enforce 1st (= 1) or 2nd (=2) stopping criteria          (default = 1)
...
% recommendations of Blanchet et al., three stopping criteria for variable
% addition are used: 1) global test of signifance, 2) permutation-based p-value
% of the variable selected in each step, and 3) the corresponding R^2adj. Note
% that automatic model building techniques are used to provide a parsimonious,
% 'optimal' subset of available variables, but are not guaranteed to find the
% 'best' subset.
%
...
% Sep-2016: now you can skip the 1st global stopping rule and still enforce the
%           others; X is standardized; 'simulation' flag is set for f_rda
...
stnd = 1; % RDA standardizes X
perm = 0; % RDA is not a permutation run
simu = 1; % RDA returns only basic statistics & p-value
% -------------------------
...
result     = f_rda(Y,X,[0],iter(1),vb,stnd,perm,simu);
glob.F     = result.F;
glob.p     = result.p;
glob.R2    = result.R2;
...
for i = 1:ng
   fprintf('  -> examining: %s \n',xLabels{i});
   result        = f_rda(Y,X(:,g(i,:)),[0],iter(2),vb,stnd,perm,simu);
...
% Re-do conditional test on best variable using global/marginal iter:
best        = f_rda(Y,X(:,g(idx,:)),[0],iter(1),vb,stnd,perm,simu);
...
for j = 2:(ng)           % 1st variable selected from Conditional tests
   ...
   for i = 1:n_g
      result    = f_rda(Y,X(:,g(i,:)),[W],[0],vb,stnd,perm,simu);
   ...
   result = f_rda(Y,X(:,g(idx,:)),[W],iter(1),vb,stnd,perm,simu);
   
   % Get cumulative fraction of TOTAL variation explained:
   result_tot = f_rda(Y,[W X(:,g(idx,:))],[0],0,vb,stnd,perm,simu); % covariables not removed
...
if (verb>0)
   ...
   if (stop>0) % Stopping rules enforced:
