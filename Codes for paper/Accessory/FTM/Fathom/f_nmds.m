...
%            unused variable 'n'
% Feb-2017: updated call to f_mantel which now outputs a structure
...
mantel    = f_mantel(dis,dist,1); % Mantel correlation
mantelRsq = (mantel.r)^2;         % rank-correlation
...
if (diag>0)
   ...
   [~,idx] = sortrows([D_vec(:),dis_vec(:)]); % [Disparities Dissimilarities]
   
   figure;
   set(gcf,'color','w');    % set bg color to white
   plot(dis_vec,dist_vec,'bo',dis_vec(idx),D_vec(idx),'r.-');
%    Rs  = f_corr(dis_vec,f_unwrap(f_dis(real(scores(:,1:2)),'euc')))^2
   grid on;
   axis square;
   axis tight;
