function [glob,marg,model] = f_rdaStepwise(Y,X,iter,verb,alpha,stop,grp,xLabels)
% - stepwise selection of explanatory variables in RDA based on F-stat
%
% USAGE: [glob,marg,model] = f_rdaStepwise(Y,X,iter,verb,alpha,stop,grp,xLabels)
%
% Y     = matrix of response variables                 (rows = obs, cols = vars)
% X     = matrix of explanatory variables
% iter  = # iterations for global/conditional permutation tests;   (default = 0)
%         input 2 values to speed up marginal tests;        e.g., iter = [1000 0]
...
%  .R2adj = adjusted R2
% 
% marg = structure of marginal effects with the following fields:
%  .F     = F-stat
%  .p     = p-value
%  .R2    = fraction of TOTAL variance explained
%  .R2adj = adjusted R2
%  .var   = variable labels
%
% model = structure of conditional effects of selected model:
%  .F     = partial F-stat
%  .p     = p-value
...
%  .var   = variable labels
%
% SEE ALSO: f_rda, f_rdaAIC, f_rdaDB_Stepwise

% -----Notes:-----
% This function is used to perform selection of explanatory variables in RDA via
% a stepwise, forward addition procedure. Variables are sequentially added by
% selecting the one that yields the largest partial F-statistic. Following the
% recommendations of Blanchet et al., (2008) three stopping criteria for
% variable addition are used: 1) global test of signifance, 2) permutation-based
% p-value of the variable selected in each step, and 3) the corresponding
% R^2adj. Note that automatic model building techniques are used to provide a
% parsimonious, 'optimal' subset of available variables, but are not guaranteed
% to find the 'best' subset.
%
% MARGINAL tests examine the independent effect of each explanatory variable
% on the response matrix Y. A second value input in ITER will use a different
...
%
% CONDITIONAL tests examine the effect each explanatory variable has on the
% response matrix Y after taking into account (partial out) the effects of
...
% is the number of tests. Blanchet et al. (2008) give an example of using this
% when a dataset is 'saturated' (# variables = n-1). In this case they spit the
...
% Oct-2003: fixed bug in default label creation
% Feb-2004: updated doc
% Feb-2008: updated doc
% Apr-2008: removed ncY, adjusted # arguments when calling f_rda; overhaul
%           according to recommendation of Blanchet et al.; added R2, R2adj,
%           global test, stopping criteria; optionally keep dummy codes of
%           categorical variables together; iter now accepts 2 inputs
% Aug-2016: renamed 'cond' to 'marg', etc. to clarify that the marginal tests
%           examine each variable independently, while the conditional test
%           consider the presence of other variables.
...
uGrp = unique(grp);  % get sorted list of unique groups of explanatory variables
...
result     = f_rda(Y,X,0,iter(1),0,0,0);
glob.F     = result.F;
glob.p     = result.p;
glob.R2    = result.R2;
...
for i = 1:ng
   fprintf('  -> examining: %s \n',xLabels{i});
   result        = f_rda(Y,X(:,g(i,:)),0,iter(2),0,0,0);
   marg.F(i)     = result.F;     % F-stat
   marg.p(i)     = result.p;     % p-value
   marg.R2(i)    = result.R2;    % fraction of TOTAL variation explained
   marg.R2adj(i) = result.R2adj; % adjusted R^2
   marg.var(i)   = xLabels(i);   % variable label
end

% Determine the single best variable:
idx = find(marg.F == max(marg.F)); % best F-ratio wins
idx = idx(1);                      % in case of ties, select one

% Re-do marginal test on best variable using global/conditional iter:
best        = f_rda(Y,X(:,g(idx,:)),0,iter(1),0,0,0);
marg.p(idx) = best.p; % replace with permuted p-value
...
% Initialize model with best variable from marginal test:
model.F(1)      = marg.F(idx);
model.p(1)      = marg.p(idx);
model.R2(1)     = marg.R2(idx);
model.R2adj(1)  = marg.R2adj(idx);
model.seqVar(1) = marg.R2adj(idx); % for 1st variable, PARTIAL explained = TOTAL explained
...
for j = 2:(ng)           % 1st variable selected from Marginal tests
   ...
   % Re-do CONDITIONAL test on best variable, but now with permutation:
   result = f_rda(Y,X(:,g(idx,:)),[W],iter(1),0,0,0);
   
   % Get cumulative fraction of TOTAL variation explained:
   result_tot = f_rda(Y,[W X(:,g(idx,:))],[0],0,0,0,0); % covariables not removed
...
if (verb>0)
   fprintf('\n==================================================\n');
   fprintf('Stepwise REDUNDANCY ANALYSIS:\n');
   fprintf('--------------------------------------------------\n');
   ...
   fprintf('Marginal Tests: (each variable separately)\n');
   headerCell = {'F','p ','R2','R2adj','Variable'}; % set up column labels
   resultCell = f_struct2flat(marg);              % flatten struct to cell array
   resultCell = [headerCell' resultCell']';       % concat
   disp(resultCell);
   fprintf('\nUsed %d permutation of residuals under a FULL model\n',iter(2));
   fprintf('--------------------------------------------------\n\n');

   fprintf('Conditional Tests: (sequential variable addition)\n');
