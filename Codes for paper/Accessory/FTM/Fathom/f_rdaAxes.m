function result = f_rdaAxes(Y,X,W,iter,verb,stnd)
% - test significance of canonical axes for RDA
%
% USAGE: result = f_rdaAxes(Y,X,W,iter,verb,stnd)
%
...
for i=a:nEvals
   ...
   % -----Permutation Test - Shuffle Residuals:-----
   if (iter>0)
      Fperm = [F(i);zeros(iter-1,1)]; % initialize
      
      % Observed reduced model:
      [~,~,~,evalsRed] = sub_rda(Y,Xres,0)
      
      
      % Get values from Y on W + Previous Axes:
      [Yfit,Yres] = sub_rda(Y,[W LC(:,1:(i-1))],1);
      
      
      idxS = (1:n)'; % get index to rows
...
%                                 SUBFUNCTION:                                 %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [U,evals] = sub_pca(X)
% - mini-version of PCA after 'f_pca'
...
% Need to return at least 1 eigenvalue:
if isempty(evals)
   evals = 0;
end
