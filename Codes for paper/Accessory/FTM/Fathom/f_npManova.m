...
% Oct-2015: replaced output variables that are ignored with ~
...
switch ncX
   case 1; % 1-Way MANOVA:
      ...
      [Q1,~] = qr(fac,0); H= Q1*Q1'; % compute Hat-matrix via QR
      m = size(fac,2);               % # of parameters in design matrix

      % just keep the result:
      [~,~,~,~,~,result] = f_npManova1(n,m,H,I,G,iter,verb);
   ...
   case 2 % 2-WAY ORTHOGONAL MANOVA:
      ...
      for i=1:2
         ...
         [Q1,~] = qr(fac{i},0); H{i} = Q1*Q1'; % compute Hat-matrix via QR
         m(i) = size(fac{i},2);                 % # of parameters in design matrix
      end

      %-----Interaction term (1x2):-----
      [~,fac{3}] = f_designMatrix(x,1);     % create ANOVA design matrix
      fac{3} = [ones(n,1) fac{3}];          % add intercept
      [Q1,~] = qr(fac{3},0); H{3} = Q1*Q1'; % compute Hat-matrix via QR
   ...
   case 3 % 3-WAY ORTHOGONAL MANOVA:
      ...
      [~,iTerms] = f_designMatrix(x,1);    % design matrices specifying interactions
      for i=4:7
