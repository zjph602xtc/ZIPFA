function [best,cond] = f_rdaDB_AIC(yDis,ncY,X,W,BIC,verb,cut,xLabels)
% - stepwise forward selection of explanatory variables in db-RDA using AIC (or BIC)
%
% USAGE: [best,cond] = f_rdaDB_AIC(yDis,ncY,X,W,BIC,verb,cut,xLabels);
%
...
%  .var   = variable labels
%
% SEE ALSO: f_rdaDB, f_rdaDB_Stepwise, f_rdaAIC, f_AIC, f_rdaStepwise
...
% May-2016: call f_rdaDB with stnd=1, perm=0, simu=1
% Apr-2017: swapped labels for MARGINAL VS. CONDITIONAL; still need to rename
%           variables

% -----Set defaults & check input:-----
if (nargin < 4), W       = []; end % default no covariables
if (nargin < 5), BIC     = 0;  end % return AIC by default
if (nargin < 6), verb    = 1;  end % send output to display by default
if (nargin < 7), cut     = 2;  end % default cut-off value of 2
...
k = 1; % 1 variable at a time for MARGINAL tests
for i = 1:ncX
   result        = f_rdaDB(yDis,ncY,X(:,i),W,0,0,1,0,1); % stnd = 1, skip = 0, simu=1
...
if (idx == ncX+1)
   best = NaN;
   fprintf('\nConditinal tests skipped: no variable is better than a null model!\n\n')
   return
end
...
Z            = X(:,idx); % initialize Z with best variable
...
for j = 2:ncX            % 1st variable selected from conditional tests
   ...
   for i = 1:nc_x
      result        = f_rdaDB(yDis,ncY,[Z X(:,i)],W,0,0,1,0,1); % stnd = 1, skip = 0, simu=1
...
if (verb>0)
   ...
   if (BIC<1)
      if isempty(W)
         fprintf('AIC-based forward addition: db-RDA \n');
      else
         fprintf('AIC-based forward addition: partial db-RDA \n');
      end
   else
      if isempty(W)
         fprintf('BIC-based forward addition: db-RDA \n');
      else
         fprintf('BIC-based forward addition: partial db-RDA \n');
      end
   end
   fprintf('--------------------------------------------------\n');
