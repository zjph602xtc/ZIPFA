...
%  .loo_bin      = leave-one-out classified correctly (= 1) or incorrectly (= 0)
...
% Jun-2016: replaced null variables with ~; no longer calls f_unique; no longer
%           calls f_shuffle
...
if (cda>0)
   if (sm>0)
      [centroids,~,mDis] = f_centroid(siteScores,grp,2); % spatial median
   else
      [centroids,~,mDis] = f_centroid(siteScores,grp,1); % centroid
   end
else
   centroids = NaN;
end
...
if ~isempty(W)
   ...
   for i = 1:nW
      ...
      [~,idxC]        = max(PP(i,:),[],2);  % get index to maximum PP
...
if iter>0
   ...
   for i = 1:(iter-1) % observed value is considered a permutation
      ...
      [~,evalsPermed] = f_eig(Qm(idxS,:)'*H*Qm(idxS,:)); % Eigenanalysis
...
if (loo>0)
   ...
   % Binary vector: 1 = correctly re-classified, 0 = incorrect
   loo_bin = ((grp - cls_o) == 0);
...
result.loo_bin      = loo_bin;      % Leave-One-Out binary vector
